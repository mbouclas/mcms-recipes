...
namespace Mcms\Recipes\Console\Commands;
...
use Mcms\Recipes\Console\Commands\InstallerActions\ApplyProvisionSettings;
use Mcms\Recipes\Console\Commands\InstallerActions\MigrateDataBase;
use Mcms\Recipes\Console\Commands\InstallerActions\PublishAssets;
use Mcms\Recipes\Console\Commands\InstallerActions\PublishSettings;
use Mcms\Recipes\Console\Commands\InstallerActions\PublishViews;
use Mcms\Recipes\Console\Commands\InstallerActions\SeedDataBase;
...
/**
 * Class Install
 * @package Mcms\Recipes\Console\Commands
 */
class Install extends Command
{
     ...
     * @var string
     */
    protected $signature = 'recipes:install {provisionFile?} {--action=}';
    ...
    public function handle()
    {
        ...
        $command = "php artisan recipes:refreshAssets";
        ...
        $this->registerEvents();
        $this->info('Recipes package is starting the installation');
        $this->line('--------------');
    ...
    {
        Event::listen('installer.recipes.run.before', function ($msg, $type = 'comment'){
            $this->{$type}($msg);
        });

        Event::listen('installer.recipes.run.after', function ($msg, $type = 'comment'){
            $this->{$type}($msg);
        });
    }
}
